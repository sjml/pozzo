...
class DB {
    ...
    static function GetAllPhotos() {
        $statement = self::$pdb->prepare("SELECT id, title, hash, width, height, size FROM photos");
    ...
    static function InsertPhoto($photoData) {
        $statement = self::$pdb->prepare("INSERT INTO photos(title, hash, width, height, size) values(?,?,?,?,?)");
